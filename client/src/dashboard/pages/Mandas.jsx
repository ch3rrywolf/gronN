...
const Mandas = () => {
  ...
  const [mandas, setMandas] = useState([]);
   const [res, set_res] = useState({ id: '', loader: false }); 

  const get_mandas = async () => {
    try {
      const { data } = await axios.get(`${base_url}/api/mandas`, {
        headers: { Authorization: `Bearer ${store.token}` },
      });
      setMandas(data?.mandas || []);
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const update_status = async (accountStatus, mandas_id) => {
    try {
        set_res({ id: mandas_id, loader: true });
        const { data } = await axios.put(`${base_url}/api/mandas/status-update/${mandas_id}`, { accountStatus }, {
            headers: {
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('Are you sure you want to delete this manda?'))
      return;

    try {
      await axios.delete(`${base_url}/api/mandas/delete/${id}`, {
        headers: { Authorization: `Bearer ${store.token}` },
      });

      setMandas((prev) => prev.filter((op) => op._id !== id));
      alert('Manda deleted successfully');
    } catch (error) {
      console.error('Error deleting manda:', error);
    }
  };
  ...
  return (
    <div className='bg-white rounded-md'>
      <div className='flex justify-between p-4'>
        <h2 className='text-xl  text-[#1960a9] hover:text-[#9fc327] font-bold'>Mandas</h2>
        <Link
          className='px-3 py-[6px] bg-[#9fc327] rounded-sm text-white hover:bg-[#1960a9]'
          to='/dashboard/manda/add'
        >
          Ajoute Mandataire
        </Link>
      </div>
              ...
              <tr key={i} className='bg-white border-b'>
                <td className='px-6 py-4'>
                  ...
                  <>
                    ...
                    <td className='px-6 py-4'>{r.nomRepParMan}{r.prenomRepParMan}</td>
                    <td className='px-6 py-4'>{r.adresseMan}</td>
                    <td className='px-6 py-4'>{r.nomRepParMan}{r.prenomRepParMan}</td>
                    <td className='px-6 py-4'>{r.numTelRepParMan}</td>
                  </>
                ...
                <td className='px-6 py-4'>
                  <div className='flex justify-start items-center gap-x-4 text-white'>
                    <Link
                      to={`/dashboard/mandas/view/${r._id}`}
