...
const gesteOptions = ["MENUISERIE FENETRE 2V", "MENUISERIE FENETRE 1V", "VMC simple flux"];
  ...
  const submit = async(e) => {
    ...
    try {
      setLoader(true)
      const {data} = await axios.post(`${base_url}/api/prestationmar/add`,state, {
        headers : {
      ...
      navigate('/dashboard/prestationmars')
    } catch (error) {
      setLoader(false)
      ...
      <div className='p-4'>
        <form onSubmit={submit}>
          <div className='grid grid-cols-3 gap-x-8 mb-3'>
            ...
            <div className='flex flex-col gap-y-2'>
              <label className='text-md font-medium text-gray-600' htmlFor='prixdevente'>Prix de vente HT  *</label>
              <input onChange={inputHandler} value={state.prixdevente} required type='text' placeholder='prix de vente' name='prixdevente' id='prixdevente' className='px-3 py-2 rounded-md outline-0 border border-gray-300 focus:border-green-500 h-10' />
            </div>
          </div>
...
export default AddPrestationmar;