...
const Entretes = () => {
  ...
  const update_status = async (accountStatus, entretes_id) => {
    try {
        set_res({ id: entretes_id, loader: true });
        const { data } = await axios.put(`${base_url}/api/entretes/status-update/${entretes_id}`, { accountStatus }, {
            headers: {
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('Are you sure you want to delete this entrete?'))
      return;

    try {
      await axios.delete(`${base_url}/api/entretes/delete/${id}`, {
        headers: { Authorization: `Bearer ${store.token}` },
      });

      setEntretes((prev) => prev.filter((op) => op._id !== id));
      alert('Entrete deleted successfully');
    } catch (error) {
      console.error('Error deleting entrete:', error);
    }
  };
  ...
  return (
    <div className='bg-white rounded-md'>
      <div className='flex justify-between p-4'>
        <h2 className='text-xl  text-[#1960a9] hover:text-[#9fc327] font-bold'>Entreprises</h2>
        <Link
          className='px-3 py-[6px] bg-[#9fc327] rounded-sm text-white hover:bg-[#1960a9]'
          to='/dashboard/entrete/add'
        >
          Ajoute Entreprise retenue
        </Link>
      </div>
              ...
              <tr key={i} className='bg-white border-b'>
                <td className='px-6 py-4'>
                  ...
                  <>
                    ...
                    <td className='px-6 py-4'>{r.raiSocEntRe}</td>
                    <td className='px-6 py-4'>{r.adresseEntRe}</td>
                    <td className='px-6 py-4'>{r.nomRepParEntRe}{r.prenomRepParEntRe}</td>
                    <td className='px-6 py-4'>{r.numTelRepParEntRe}</td>
                  </>
                ...
                <td className='px-6 py-4'>
                  <div className='flex justify-start items-center gap-x-4 text-white'>
                    <Link
                      to={`/dashboard/entretes/view/${r._id}`}
